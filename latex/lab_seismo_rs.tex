...
\item Open \verb+run_getwaveform_626.m+ (shown in \refApp{sec:script}) and \verb+plotw_rs.m+.
%
\begin{enumerate}
\item Run \verb+run_getwaveform_626.m+ to produce the map in \refFig{fig:map_ex01} and the record section in \refFig{fig:rs_ex01}. \\
...
\item Comment the \verb+break+ statement at the end of \verb+run_getwaveform_626.m+, and then reproduce the spectrogram in \refFig{fig:BOOM} by following the 7 steps within the Matlab comments.

\item Uncomment the \verb+break+ statement to turn off the spectrogram tool. If you want to see spectrograms for other stations in other examples, then you need to replace \verb+BOOM+ with another station name.
...
\item the widest period range over which the signal is clearly visible. This can be achieved by varying \verb+T1+ and \verb+T2+ when calling \verb+plotw_rs.m+: you do not need to re-extract the waveforms.
\end{itemize}
%
\begin{table}[h]
\centering
