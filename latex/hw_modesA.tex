...
\cltag\ \\
Assigned: February 17, \cyear\ --- Due: February 26, \cyear\ \\
Last compiled: \today \\
\end{spacing}
...
\item This problem set provides a hands-on introduction to how normal modes of a particular earth model can be computed. The goal is to gain a better understanding for the concepts of normal modes, eigenfrequencies, eigenfunctions, dispersion, and numerically solving equations.
...
\item (0.0) Now examine the output and the three subplots within the $3 \times 3$ subplot figure at the bottom.
%
\begin{enumerate}
...
\item (0.7) Set \verb+plot_all_eigenfunctions = False+ to suppress the inital two figures.

Set \verb+fmax = 0.08+ and rerun. Examine the $3 \times 3$ subplot figure.
%
\begin{enumerate}
...
%
\begin{enumerate}
\item (0.3) In \refFig{fig:dots}b, plot (by hand) a dot for each function evaluation $(l, f)$ that you performed. (But ignore all the function evaluations that occur within the root-finding procedure.) Put one star on the first $(l,f)$ evaluation and one square on the final $(l,f)$ evaluation.
...
\item (0.1) On your plot, mark the wavelength $\lambda$ as a distance from the surface.
\end{enumerate}
...
\item (0.2) What model parameters would you search over if you wanted to obtain an optimal linear earth model?
\end{enumerate}
...
Qualitatively, how does this plot compare with the predictions for PREM, shown in \refFig{fig:dispersionApp}c?
\end{enumerate}
...
angular frequency & $\omega$ & \hspace{2cm}mrad/s & same  & \hspace{2cm}mrad/s & same \\  \hline
...
The calculations are based on PREM \citep{PREM}, not a homogeneous Earth model.
\label{fig:dots}
}}
\end{figure}
