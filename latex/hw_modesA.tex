...
Assigned: February 26, 2018 --- Due: March 9, 2018 \\
Last compiled: \today \\
\end{spacing}
...
\item The necessary starting Matlab scripts and data are available in the class github directory. The relevant files are:
%
\begin{itemize}
...
\clearpage\pagebreak
\input{/home/carltape/REPOSITORIES/classes/seismo/latex/PREM_dispersion_fig}
...
\item (0.0) Execute the script \verb+spshell.m+ and analyze the output. Two figures are generated, one for a set of $W(r)$, the other for a set of $T(r)$ (\refFig{fig:Tr}).
%
\begin{enumerate}
...
\item (0.0) Now change \verb+iplot_all_freqs+ from 1 rto 0, then rerun.
%
\begin{enumerate}
...
Note: See code at the bottom of \verb+spshell.m+ to read in the observations.
...
\item (0.4) Compute $F(\bem)$ for all available observations (\refTab{tab:modes_obs}). These observations will be loaded within \verb+spshell.m+ if you comment out the last \verb+break+ statement.

Tip: Initialize an $a \times b$ matrix as \verb+Fobs = NaN(a,b)+, then fill certain entries according to $n$ and $l$ listed in the observations. This ``observations matrix'' can be differenced with a corresponding ``predictions matrix''; the residual matrix can be turned into a vector as \verb+Fres(:)+. See example code at the bottom of \verb+spshell.m+.
