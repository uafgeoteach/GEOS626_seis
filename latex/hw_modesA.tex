...
%
\begin{itemize}
\item \verb+spshell.ipynb+ (\refSec{sec:shell})
\item \verb+surf_stress.py+ (\refSec{sec:surf})
\item \verb+stress_disp_tor.py+ (\refSec{sec:stress})
\end{itemize}
...
\caption[]
{{
Solving the eigenvalue problem for the vibrating shell is accomplished using three scripts. The main purpose of each script is described in the above chart, which also shows the dependencies: {\tt spshell.ipynb} calls {\tt surf\_stress.py}, which requires {\tt stress\_disp\_tor.py}.
\label{fig:workflow}
}}
\end{figure}
...
\item (0.0) Execute the script \verb+spshell.ipynb+ and analyze the output. Two figures are generated, one for a set of $W(r)$, the other for a set of $T(r)$ (\refFig{fig:Tr}).
%
\begin{enumerate}
...
Note: See code at the bottom of \verb+spshell.ipynb+ to read in the observations.
...
\item (0.4) Compute $F(\bem)$ for all available observations (\refTab{tab:modes_obs}). These observations can be loaded at the bottom of \verb+spshell.ipynb+. List your answer in mHz with 0.001 precision.

Tip: Initialize an $a \times b$ matrix as \verb+Fobs = np.full((a,b),np.nan)+, then fill certain entries according to $n$ and $l$ listed in the observations. This ``observations matrix'' can be differenced with a corresponding ``predictions matrix''; the residual matrix can be turned into a vector as \verb+Fres.flatten(order='F')+. See example code at the bottom of \verb+spshell.ipynb+.
...
\item (1.0) {\bf Linear earth.} Adapt \verb+earthfun.py+ (\refSec{sec:earthfun}) to create a function that inputs a radial value $r$ (or a vector of $r$ values) and outputs $\rho(r)$ and $\mu(r)$ described by a linear model with the ``endpoint'' values
%
\begin{eqnarray*}
...
Test your \verb+earthfun.py+ for a few values of $r$ to make sure that the output values of $\rho$ and $\mu$ are sensible (no work needed).

%Note: As suggested in \verb+earthfun.m+, it may be helpful to use \verb+rspan+ as a global variable.

\begin{enumerate}
\item (0.4) Show your code for \verb+earthfun.py+.
...
degree & $l$ & \hspace{2cm} & \blank & \hspace{2cm} & \blank \\  \hline
