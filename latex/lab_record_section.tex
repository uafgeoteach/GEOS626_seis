...
\item The two main procedures are executed using the software package PySep (\url{https://pysep.readthedocs.io/en/latest/index.html}), which uses ObsPy \citep{obspy2010,obspy2011,obspy2015}.
...
\item Run \tfile\ to produce the map and record section (\refFigii{fig:map_ex01}{fig:rs_ex01}). \\
...
% ZZZ master branch documentation
\scriptsize
\url{https://pysep.readthedocs.io/en/latest/autoapi/pysep/recsec/index.html#pysep.recsec.RecordSection}
\normalsize
...
\item Example 2: near-source recordings of \magnum{\magMw}{7.5} earthquake in SE Alaska. 
...
\item Example 6: Sumatra \magnum{\magMw}{8.6} triggering earthquakes in Alaska. 
%
\begin{itemize}
...
\item Change the bandpass period range (\verb+min_period_s+ and \verb+max_period_s+) for the record section plot to be \trange{2}{1000}, so that you see the complete frequency range of this waveform. \\
Approximately how long did this earthquake last in Alaska?
\end{itemize}

\item Example 6.1, 6.2 and 6.3: other earthquakes during the Sumatra \magnum{\magMw}{8.6} earthquake in Alaska.

You are given the source parameters for three earthquakes that occurred in Alaska during the ground motion of the main wavetrain from the Sumatra \magnum{\magMw}{8.6} earthquake.
...
\item the widest period range over which the signal is clearly visible. This can be achieved by varying \verb+min_period_s+ and \verb+max_period_s+ provided as an input for plotting the record sections. You do not need to re-extract the waveforms.
\end{itemize}
%
\begin{table}[h]
\centering
\caption[]
{{
Possibly triggered earthquakes during the 2012 Sumatra \magnum{\magMw}{8.6} earthquake in Alaska. List the bandpass range that you used to make your assessments.
\label{tab:triggered}
}}
\begin{spacing}{1.5}
...
\item Now play around by changing any of the parameters for plotting record sections and making spectrograms for the provided examples. 

\item Example 7: Try your own example. Take a look at any event of your interest by extracting waveforms, plotting a record section, and looking at spectrograms.
