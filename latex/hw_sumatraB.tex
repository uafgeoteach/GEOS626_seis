...
\item (0.4) Run \verb+lab_sumatraB.ipynb+ and examine the stations that were cut from the analysis. Describe four ``errors'' in these records, and list at least one station associated with each error.
...
\begin{enumerate}
% ZZZ THIS IS ALREADY DONE IN THE LAB.
\item (0.4) Using your list of waveform IDs from \verb+lab_sumatraB.ipynb+, recompute the spectra and plot them.
...
\item You do not need to compute the smoothed envelopes---the bandpassed seismograms are enough. (Though computing the envelopes is straightforward; see \verb+hwsol_sumatraA.pdf+.)
\end{itemize}
