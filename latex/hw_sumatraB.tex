...
\cltag\ \\
Assigned: March 3, \cyear\ --- Due: March 17, \cyear\ \\
Last compiled: \today
\end{spacing}
...
\caption[]
{{
Rupture model for the 2004-12-26 \magnum{\magMw}{9.2} Sumatra earthquake.
...
\item \verb+lab_sumatraB.pdf+, on sumatra waveforms. This also includes an overview of the Global Seismograpphic Network (GSN), which is composed of four networks: IU, II, IC, and CU.
\end{enumerate}
...
\item {\bf Important:} For any script, if you change the ``fetch'' settings for getting waveforms, then you willw ant to manually delete the previous files in your folder. Otherwise your analysis will contain waveforms that were fetched with two different settings.
...
\medskip\noindent
Review from lab: Run \tfilelab\ and examine the stations that were cut from the analysis. Describe four ``errors'' in these records, and list at least one station associated with each error.

%\medskip\noindent
%NOTE: Do NOT log-scale the frequency axis in this problem.
...
List the median amplitude value (not the frequency).
\end{enumerate}
...
\item The travel time between the fault and any station is encapsulated with the simple velocity $v = 11$~km/s. This is the mean apparent velocity for stations between $30^\circ < \Delta < 85^\circ$ of the source. Given an arc-distance of $d = a\Delta$~km from source to station ($a$ is Earth radius, $\Delta$ is in radians), the P wave traveltime is approximately $d/v$~seconds.
 %, assuming Jeffreys--Bullen P travel times and arc distances (not distances along the P wave ray path).
...
\end{equation}
%
where $v_r$ is the rupture velocity, $v$ is the velocity of the medium (11~km/s), $L$ is the fault length, $\alpha$ is the azimuth to the station, and $\alpha_0$ is the rupture direction. In answering the following questions, note that {\bf no numbers are needed (only algebra).}
%
\begin{enumerate}
...
%\pagebreak
\item (1.6) This problem can be completed without having finished the previous steps.
\begin{enumerate}
\item (1.0) With \citet[][Figure~1d]{Ni2005} as a guide, modify \tfilehf\ to produce a record section of filtered seismograms. (In part (c), you will work with envelopes.)
...
\item Use stations within the epicentral distances given by $30^\circ < \Delta < 85^\circ$. (You will only find this detail in the Supplement of \citet{Ni2005}). You may need to delete any previously saved data files, if you are seeing stations having epicentral distances outside of the desired range.
...
\begin{enumerate}
\item (0.4) Modify \tfilelf\ to \textbf{generate a record section} that shows that the directivity can also be inferred from the relative amplitudes of R1 and R2, the ``minor orbit'' and ``major orbit'' Rayleigh wave arrivals\footnote{See Figure 2.7-3 of \citet{SteinWysession}.}. See Figure S1 of \citet{Ammon2005} as a guide.

Your record section should have at least 3 seismograms.
  
% ZZZ PYTHON ZZZ
Optional 1: Use a larger subset of stations to demonstrate the key point. (What distance range do you want to specify?)
  
Optional 2: Quantify the amplitude ratios by using the log-scaled quantity $\ln(A_{R1}/A_{R2})$, where $A_{R1}$ is the amplitude of the filtered R1 wave. This will require designating time windows for R1 and R2, then calculating the maximum.
...
Note that here we ask for network IC\footnote{IC is also part of the Global Seismographic Network, along with II, IU, and CU.}, which will provide station IC.LSA.
...
%
\begin{enumerate}
\item (0.4) Include plots of bandpass-filtered seismograms (or a record section) for instrument-response deconvolved.

Tip: If you encounter bad data, consider choosing between different location codes (e.g., 00 vs 10).

\item (0.4) Include plots of bandpass-filtered seismograms (or a record section) for raw data. You may have to re-download the data.

Tip: To enable comparisons, you will want to integrate the filtered raw recordings to dispalcement.

\item (0.2) How different are the filtered waveform shapes between the raw and instrument-response-deconvolved versions?
