...
\begin{enumerate}
\item Examine \tfile. For the LHZ raw data, list the parameters specified in the request:
...
\item Run \tfile\ to fetch the database of waveforms and save them as local files.
%If successful, you should see a large set of seismograms displayed.

Lots of figures will be generated, but we will focus here only on the first part.

Your seismograms are saved locally in the folder \verb+datawf+ in subfolders \verb+sumatra_LHZ_raw+.

\item Examine the list of rejected waveform IDs. Explain the four parts that make up each waveform ID. (See also \verb+lab_record_section.pdf+.) Note that the order of the four parts is critical.

\item Examine the seismograms that were rejected from the analysis. Describe three ``errors'' in these seismograms.

%Open \verb+sumatra_modes.txt+ and make sure you understand what is shown.

\item Browse all remaining waveforms and pick a subset of {\bf at least 20 stations} for the analysis of normal modes.
...
\item Run \tfile\ and plot your subset of seismograms. \\
This will prepare you for \verb+hw_sumatraB.pdf+.

\item The next set of data will have the response to acceleration removed. Your seismograms will be saved locally in the subfolder \verb+sumatra_LHZ_acc+. Acceleration spectra are preferred output for analyzing normal modes.

\item The next set of data will have the response to acceleration removed. Your seismograms will be saved locally in the subfolder \verb+sumatra_BHZ_vel+.
