...
\item The key Python functions for the lab are \verb+scipy.signal.hilbert()+, \verb+numpy.fft.fft()+, and \verb+numpy.fft.ifft()+.
%Also required are \verb+scipy.signal.butter()+ and \verb+scipy.signal.filtfilt()+ for bandpass filtering.
...
\item Using \verb+bandpass()+, compute a bandpass-filtered version of the PAS time series. Given a target period of $f_0 = 1/20$~Hz, use the bandpass frequency limits of $f_a = f_0(1-p)$ and $f_b = f_0(1+p)$ where $p = 0.2$.
