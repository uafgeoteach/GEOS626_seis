...
\item This lab uses ObsPy \citep{obspy2010,obspy2011,obspy2015}, within the software package PySep (\url{https://adjtomo.github.io/pysep/}), to access seismic waveforms from the IRIS Data Management Center. See {\tt lab\_response.pdf}.
...
%
\begin{itemize}
\item Change \verb+networks+ to \verb+'AK'+ and rerun the example.
...
\item Change the bandpass period range (\verb+min_period+ and \verb+max_period+) for the record section plot to be \trange{2}{1000}, so that you see the complete frequency range of this waveform. \\
...
\item the widest period range over which the signal is clearly visible. This can be achieved by varying \verb+min_period+ and \verb+max_period+ provided as an input for plotting the record sections. You do not need to re-extract the waveforms.
\end{itemize}
%
\begin{table}[h]
\centering
