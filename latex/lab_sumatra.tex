...
\begin{spacing}{1.2} 
\centering
{\large \bf Lab Exercise: Waveforms from the 2004 Sumatra-Andaman earthquake} \\
...
\begin{itemize}
\item This lab is a starter for the homework on the Sumatra earthquake. The goal is to understand the attributes of a seismic waveform (start time, end time, sample rate, station location, etc) and to gain some appreciation for variability in the quality of waveforms, especially when recording a \magw{9.0} earthquake, which puts some seismometers to a difficult test!
...
\end{spacing}
%
(You can see these lines when running \verb+sumatra_hf_template.m+.)
...
\begin{enumerate}
\item  Run \verb+sumatra_modes.m+ and examine the stations that were cut from the analysis. Describe three ``errors'' in these records.

\item From a terminal window (not Matlab), use \verb+xpdf+ or \verb+evince+ to browse the 139-page pdf file
...
\item Run \verb+sumatra_modes.m+ and plot your subset of records.
...
%\pagebreak
\section{Building the database of waveforms [FOR READING ONLY]}
