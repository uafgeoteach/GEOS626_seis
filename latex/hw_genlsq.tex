...
\cltag\ \\
Assigned: April 9, \cyear\ --- Due: April 16, \cyear\ \\
Last compiled: \today
\end{spacing}
...
\end{eqnarray*}
%
The posterior model is the same as the final model of an iterative problem. And the ``target model'' is the same as the ``true model'': this is the model used to generate the ``target data'' or ``true data''. We will use the notation $\mpost$ and $\mtarget$. The variable $n$ refers to both the number of observations and the index of the iterative inversion.
...
%
\begin{eqnarray}
\mpost &=& \left(\bG^T \covdi \bG + \cpriori \right)^{-1}
\left( \bG^T \covdi \bd + \cpriori\mprior \right)
\label{mpost1}
\\
&=& \mprior + \left(\bG^T \covdi \bG + \cpriori \right)^{-1} \bG^T \covdi \left( \bd - \bG \mprior\right)
\label{mpost2}
\\
&=& \mprior + \cprior \bG^T \left( \bG \cprior \bG^T + \Cd\right)^{-1} \left( \bd - \bG \mprior\right)
\label{mpost3}
\end{eqnarray}
%
HINT: You may find it useful to use the two matrix identities in \citet[][Eq.~6.525]{Tarantola2005} (which are derived in \verb+notes_tarantola.pdf+), as well as the expressions for the posterior covariance,
%
\begin{eqnarray}
\cpost &=& \left(\bG^T \covdi \bG + \cpriori \right)^{-1}
\label{cpost1}
\\
&=& \cprior - \cprior \bG^T \left( \bG \cprior \bG^T + \covd \right)^{-1} \bG \cprior
\label{cpost2}
\end{eqnarray}
...
You should generate four figures, one of which is \refFig{fig:srcrec}. Then the code will crash because some code still needs to be written.
...
\item (0.2) What do the functions \verb+g()+, \verb+d()+, \verb+G()+, and \verb+S()+ do? Put your responses in \refTab{tab:funs}.
...
\item (0.1) From Section 3.2.3 \citep{Tarantola2005}, what is the equation for $\cpost$?
...
\item (0.5) 
\begin{enumerate}
\item (0.1) In the space provided in \tfile, use $\cpost$ to compute the posterior correlation matrix. Show your lines of code, and display the correlation matrix. A Python function equivalent to the Matlab command \verb+corrcov+ is provided to check your code.
...
%
\begin{enumerate}
\item (0.1) Based on visual inspection of the convergence curves in this example, how many iterations are needed for convergence? \\
