...
\cltag\ \\
Assigned: January 13, \cyear\ --- Due: January 22, \cyear\ \\
Last compiled: \today
\end{spacing}
...
\item $M$ is the earthquake magnitude; we take this to be moment magnitude $\magMw$
...
\begin{itemize}
\item The {\em cumulative distribution} is the number of events per magnitude interval $M \ge M_i$; the \fmag\ relation is expressed as a cumulative distribution.
...
\item (0.1) What is the smallest magnitude and largest magnitude in the catalog? List your magnitudes with 0.001 precision\footnote{The estimated uncertainty of magnitudes is generally 0.1 or 0.2, but in our case want to know which magnitude bin a value will fall into, so we need to be careful about rounding.}.
\end{enumerate}

\item (0.8) Running \tfile\ will call the function \verb+seis2GR.py+ to obtain the cumulative and incremental distributions for the $\magMw$ values of the GCMT catalog, using a bin width of $\Delta M = 0.1$. Examine the output that appears.
...
\begin{enumerate}
\item (0.0) Examine the plot in \citet[][Figure~4.7-2]{SteinWysession}. You are aiming for something very close to this.

Using the output from \verb+seis2GR.py+ (provided in \tfile), {\bf plot the cumulative and incremental distributions on the same plot}, similar to the plot in \citet[][Figure~4.7-2]{SteinWysession}, but note that your $x$-axis is $\magMw$, not $\log_{10} M_0$, and your $y$-axis is number of earthquakes, not number of earthquakes per year.

{\em Coding tip}: There are different ways to implement log scaling; one is provided at the bottom of \tfile.
%There are two ways to deal with log scaling.One way is to transform $N$ into $n = \log_{10} N$, then work with~$n$ and use the \verb+plot+ command. Another way is to work with $N$ directly and use the \verb+semilogy+ command, as shown in the example at the bottom of \tfile.
...
\item (0.1) From the actual data set, what magnitude interval averages $>$100 events per year?

\item (0.1) Given your $a$ and $b$ values, how many $M \ge 0$ earthquakes are expected on Earth per year?

\item (0.1) Given your $a$ and $b$ values, how many earthquakes (of any magnitude) are expected on Earth per year?
\end{enumerate}
