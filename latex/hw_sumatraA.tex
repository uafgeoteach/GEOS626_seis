...
\item (0.5) Analyze the raw 9.5-day time series of the Sumatra earthquake recorded at station CAN (Canberra, Australia), which is shown in \refFig{fig:seis}. We will use the notation $c(t)$ ($c$ is ``counts'') to represent the time series.
...
\item (0.1) Plot $|X_a(\omega)|$ over the frequency range $[0.2,1.0]$~mHz without log scaling, and compare it with $|C(\omega)|$. What is the effect of the deconvolution on the relative amplitudes of the peaks?
\end{enumerate}
