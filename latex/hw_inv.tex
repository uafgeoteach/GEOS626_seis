...
\cltag\ \\
Assigned: March 26, \cyear\ --- Due: April 2, \cyear\ \\
Last compiled: \today
\end{spacing}
...
\item {\bf excerpts} from the class notes ``Taylor series and least squares'' (\verb+notes_taylor.pdf+)
\end{itemize}
...
\begin{enumerate}
\item (0.6) Write a function that inputs a vector of radial (or depth) values and outputs a vector of $\vs$ values; include your code.
...
Inside the template notebook \tfile, write a function \verb+[x,y] = get_ellipse_points(a,b,c)+ that returns the \verb+x+ and \verb+y+ values that define the ellipse. Plot the result for the model $\bem = (0.1,\,0.3,\,0.5)$. Use the command \verb+plt.axis([-5, 5, -5, 5])+ for your plot, and ensure that the plotted region is exactly square, such that a plotted circle will appear as a cirlce.
...
\item (0.6) Starting with your expression for $F(\bem)$ (in terms of $a$, $b$, etc), derive expressions for the gradient $\bgamma(\bem)$ and the Hessian $\bH(\bem)$ of $F(\bem)$. You will need partial derivatives.

%By applying partial derivatives, determine expressions for the gradient $\bgamma(\bem)$ and the Hessian $\bH(\bem)$.

\end{enumerate}

Hint: See the class notes on the linear regression example (\verb+notes_taylor.pdf+, Section 3.2).

%-----------------

\item (1.0) Show that your explicit expressions for $\bgamma(\bem)$ and $\bH(\bem)$ (in terms of $a$, $b$, etc) can be expressed as
%
\begin{eqnarray}
...
\item (1.0)
\begin{enumerate}
\item (0.5) Implement your result in 3-1-c and solve for $\bem$ using the data
%
\begin{equation*}
(x_i,\;y_i) \;:\; (3,3),\;(1,-2),\;(0,3),\;(-1,2),\;(-2,-2),\;(0,-4),\;(-2,0),\;(2,0).
\end{equation*}

Tip: Define a function \verb+get_ellipse_model()+ that will return $a$, $b$, and $c$.
...
There is a cell in \tfile\ that allows you to choose points on a plot, and saves the points to two arrays.
