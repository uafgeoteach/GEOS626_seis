...
%
\begin{itemize}
\item You will need to rename \verb+spshell.ipynb+ to \verb+spshell_love.ipynb+ and then make significant modifications. 
...
%\end{verbatim}
%
Note that \verb+rvec+ and \verb+WT+ are defined in \verb+surf_stress_love.py+, not \verb+spshell_love.ipynb+.
...
\begin{enumerate}
\item (0.0) Given our coordinate system, our \verb+rspan+ will now change depending on the input period $T$ (or $f$ or $\omega$). What are your new values for \verb+rspan[0]+ and \verb+rspan[1]+? Label \verb+rspan[0]+ and \verb+rspan[1]+ in your sketch from Problem~1-\ref{prob:setup}.
...
\item (0.5) In $\sim$3 sentences, explain what \verb+spshell_love.ipynb+ is doing in order to find $k_0$.
...
\item (1.0) Check the system of equations (\refeq{rODEs}) for this solution (see modes homework solutions). You should get much better agreement if you lower the tolerance levels for \verb+ode45+ by changing \verb+max_step+ in the second cell.
