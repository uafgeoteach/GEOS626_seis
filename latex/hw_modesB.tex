...
\cltag\ \\
Assigned: March 17, \cyear\ --- Due: March 26, \cyear\ \\
Last compiled: \today
\end{spacing}
...
This problem set is a direct extension of the problem set ``Toroidal modes of a spherically symmetric earth'' (\verb+hw_modesA+). Here we examine mode solutions for Love waves of a simple model of a layer over a halfspace. The layer represents the crust, and the halfspace represents the upper mantle.\footnote{You might find it easier to think of this as a two-layer model. Technically the crust is the only part that has an upper and lower limit, so this is why it is considered the only layer.}
...
\begin{itemize}
\item $\omega_{x0} = 0$~Hz.
\item The index $\bar{n} \ge 1$ is the number branches ($n = 0$, $n = 1$, \ldots, $n = \bar{n}-1$) that are present for the frequency interval
%$\omega_{x\overline{(n-1)}} < \omega < \omega_{x\bar{n}}$.
$\omega_{x{(\bar{n}-1)}} < \omega < \omega_{x\bar{n}}$.
\item For $\bar{n} = 1$, there is one branch ($n=0$) for the interval $0 < \omega < \omega_{x1}$.
\item For $\bar{n} = 2$, there are two branches ($n=0$, $n=1$) for the interval $\omega_{x1} < \omega < \omega_{x2}$.
\item Therefore, for any frequency $\omega$, one can determine $\bar{n}$ from interval $\omega_{x{(\bar{n}-1)}} < \omega < \omega_{x\bar{n}}$, and this provides the number of mode branches expected.
\item The $n$th mode branch starts at $\omega_{x\bar{n}}$. The $n=0$ branch starts at $\omega_{x1} = 0$~Hz, the $n=1$ branch starts at $\omega_{x1}$, etc.
\end{itemize}
%
...
\item (0.3) Make a 2D sketch the setup for this problem and include the following features and labels: the layer, $r$-axis ($r$ defined in \refeq{rODEs}), $r$-origin, $H$, $\lambda_m$, $\rho_c$, $\mu_c$, $\rho_m$, $\mu_m$.
...
\item (0.0) You will examine two additional models. Fill in \refTab{tab:models} with the values for these models. (See \refTab{tab:models} caption.)
...
%
\begin{itemize}
\item You will need to make significant modifications to \tfilemainB, which is provided as a starting point. (Alternatively, you can use your own notebooks as a starting point.)
...
Examine \tfilessB\ to see what arguments are needed for \tfilesdtB.
...
\begin{enumerate}
\item (0.5) Check that your root is $k_0 = 8.52 \times 10^{-5}$~1/m (Note \footnote{If your value does not round to the one listed, what might you consider changing?}) and show the line in your Python script that solved for $k_0$.
...
\item (1.0) Show a plot of the two eigenfunctions. The base code for plotting eigenfunctions can be found in \tfilemainB\ and needs to be adapted for the new purposes.

\begin{itemize}
\item Draw a line at the surface and at the base of the layer (i.e., the Moho) to assist with interpretations.

\item If the eigenfunctions appear jagged, then adjust \maxstep\ to improve their accuracy.

\item Suggestion: For plotting purposes only, you may find it simpler to transform your $r$~values into depth values, with $z=0$ at the surface. You can then use \\ \verb+plt.invert_yaxis()+ to flip the direction of the $y$-axis.
\end{itemize}
...
%
\begin{itemize}
\item (0.4) phase speed~(km/s) vs\footnote{Reminder: ``A vs B'' means that A is on the $y$-axis and B is on the $x$-axis.} period~(s)
...
\item For numerical differentiation of $A(x)$ to get $dA/dx$, try \verb+np.gradient(A,x)+.
\end{itemize}
...
(a) Dispersion plot for toroidal modes in a homogeneous shell. The dots shows the only allowable solutions; the curves are drawn to show the difference branches ($n=0$, $n=1$, etc). 
