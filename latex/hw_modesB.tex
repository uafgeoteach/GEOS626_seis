...
\item (0.0) Given a period $T = 20$~s ($f = 1/T$), plot and label the following points in \refFig{fig:dots}: $(k_m, f)$, $(k_0, f)$, $(k_c, f)$. The notation $k_0$ is $k_n$ for $n=0$: the solution for the fundamental mode branch for a given frequency.
...
\item (1.0) Check the system of equations (\refeq{rODEs}) for this solution (see modes homework solutions). You should get much better agreement if you lower the tolerance levels for \sfind\ by changing \verb+MAX_STEP+ (in \tfileAA).
