...
\item {\bf Unit vectors.} The hat notation, like $\brh$, denoted a unit vector. If we rotate a unit vector, we still have a unit vector.
...
\item Treat everything as unit vectors ($\bK = \bKh$, $\bN = \bNh$, $\bS = \bSh$).
...
\item The angle between $\bnh$ and $\bK$ is $\kappa$.
\item The angle between $\brh$ and $\bN$ is $\theta$. (Always true)
\item The angle between $\bK$ and $\bS$ is $|\sigma|$. (Always true)
