...
\cltag\ \\
Assigned: January 29, \cyear\ --- Due: February 5, \cyear\ \\
Last compiled: \today
\end{spacing}
...
\end{equation}
%
For a double couple, $M_0 = \sfrac{1}{\sqrt{2}}\sqrt{(\lambda)^2 + (0)^2 + (-\lambda)^2} = \lambda$.
}).
...
%
\begin{enumerate}
\item (0.5) With \refFig{fig:rot} as a guide, describe (in words and symbols) the five operations that comprise $\Umat(\rotvec(\phi,\theta),\rotangB)$.

Do not use the words clockwise, counterclockwise, positive, or negative. Symbols like $-\theta$ are permitted.
...
\end{equation}
%
Here you want to write similar mathematical expressions for $\bKh$, $\bNh$, $\bSh$.
(What are $\br$, $\rotvec$, $\rotangB$, and $\br_0$ for each case?)
Please also describe each expression in a complete sentence.
%
\begin{enumerate}
...
If you are determined to get the correct numerical values, here is a check for $\bKh$, $\bNh$, and $\bSh$:
%
\begin{verbatim}
K =
   -0.0000
...
\end{verbatim}
%
However, if you did not get these numbers, you can still use them to answer the next sub-problems.
...
\pagebreak
\subsection*{Problem 3 (1.7). From fault parameters to moment tensors, Part II}

\begin{enumerate}

\item (0.7) From Problem 2, you obtained the moment tensor $\Mmat$ for the fault in \refFig{fig:cmt}.

\begin{enumerate}
\item (0.2) Numerically check that the following operations are true for this example:
%
\begin{eqnarray}
...
\begin{enumerate}
\item (0.3) List the entries of $\Mmat_{\rm denali}$ as a $3 \times 3$ matrix. What are the units?
...
\end{eqnarray*}
%
 Verify that the two moment tensors are the same (to two significant figures or so). Note \footnote{The GCMT catalog lists angles rounded to integer values. Their moment tensor entries $M_{ij}$ are listed with greater precision, so I use the $M_{ij}$ values to obtain the higher-precision fault angles ($0.01^\circ$ instead of $1^\circ$).}.
...
%\pagebreak
\subsection*{Problem 4 (0.3). Changing the coordinates of a moment tensor}

\ptag\ Compute the transformation matrix $\Pmat$ that will convert the coordinates of vectors and tensors from the up-south-east basis (unprimed) to the north-east-down (primed) basis of \citet{AkiRichardsE2}.
%
\begin{enumerate}
\item (0.1) Compute $\Pmat$ using Eq.~1 of Section A.5.1 of \cite{SteinWysession}.

\item (0.1) Calculate the fault vectors $\bKh'$, $\bNh'$, $\bSh'$ using $\bKh' = \Pmat\bKh$, etc.
