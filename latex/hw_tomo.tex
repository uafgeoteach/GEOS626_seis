...
\cltag\ \\
Assigned: April 2, \cyear\ --- Due: April 9, \cyear\ \\
Last compiled: \today
\end{spacing}
...
Using \refEqii{dlnv}{ray_tomography}, write an expression for $\delta T_i$ that has the index notation form of matrix-vector multiplication.

\item (0.5) Show how \refEq{ray_tomography} can be expressed as
%
\begin{equation}
\bdelta\dvec = \bdG\,\bdelta\bem,
\end{equation}
%
where the data vector is given by
%
\begin{equation}
\bdelta\dvec = (\delta T_1,\ldots,\delta T_i,\ldots,\delta T_\ndata)^T,
\label{d}
\end{equation}
%
...
Write the expression for the $\dG_{ik}$ entry of the partial derivatives matrix~$\bdG$.
\end{enumerate}
...
\item (2.0) {\em Compute an entry $\dG_{ik}$ of the partial derivatives matrix}. 

Modify \tfileGik\ to compute the $\ndata \times \nparm$ (ndata $\times$ nparm) partial derivatives matrix~$\bdG$.
...
%The function \verb+arcdist+ will compute the great-circle distance (in degrees) between two points on a sphere.
...
\end{verbatim}
%
Here index $i$ is the row index (or measurement index; $i = 1,\ldots,\ndata$) for $\bdG$, \verb+isrc+ is the source index ($1, \ldots, N_{\rm src}$), and \verb+irec+ is the receiver index ($1, \ldots, N_{\rm rec}$). For example, row $i=3293$ of $\bdG$ (and $\bdelta\dvec$) corresponds to the pairing of source 25 with receiver 125: $i=(25-1)132 + 125$.
...
\begin{enumerate}
\item (0.2) What are the units for $\delta d_i$, $\delta m_k$, and $\dG_{ik}$?

\item (1.8) Calculate $\dG_{ik}$ with $i=126$ and $k=204$, and check that it is $-10.3747$. \\ Show your code for calculating $\dG_{ik}$ and also your numerical output value.
...
Note: Python is 0-indexed, so the $i$th row in the $\bG$ array will be accessed using $i-1$, and the $k$th column in the $\bdG$ array will be accessed using $k-1$.

\end{enumerate}

%-----------------

\item (1.0) {\em Compute the $\ndata \times \nparm$ partial derivatives matrix $\bdG$}.

\begin{itemize}
\item Use all $\ndata=3300$ measurements and $\nparm=286$ spherical spline basis functions.

\item When properly coded, it should take $<$0.1~s to compute a row of $\bdG$ and therefore about 3~minutes to compute the entire matrix. Make sure that your code is reasonably fast (though getting the right answer is most important).
\end{itemize}

\begin{enumerate}
\item (0.6) Show your code. Show that the $(i,k)$ entry of $\bdG$ matches the value you obtained previously.

\item (0.2) What does each row of $\bdG$ correspond to?

\item (0.2) What does each column of $\bdG$ correspond to?
\end{enumerate}
...
%
\begin{itemize}
\item At this point, you should have saved your partial derivatives matrix $\bdG$. \\ Now modify \tfiletomo.
...
\item (0.5)
\begin{enumerate}
\item (0.1) Write the symbolic expression for the solution $\bdelta\bem$ to the least squares problem $\bdG\bdelta\bem=\bdelta\dvec$ in the case where $\bdG^T\bdG$ is full rank.

\item (0.4) List the dimensions and units of $\bdelta\bem$, $\bdelta\dvec$, $\bdG$, $\bdG^T\bdG$, and $\bdG^T\bdelta\dvec$.
\end{enumerate}
...
\item (1.3) If $\bdG^T\bdG$ is not full rank, then a damping parameter $\lambda$ may be introduced to stabilize (or ``regularize'') the inversion.  One possible solution to the problem is:
%
\begin{equation}
\bdelta\bem_{\lambda} = \left(\bdG^T\bdG+\lambda^2\bI\right)^{-1}\bdG^T\bdelta\dvec.
\end{equation}
%
\begin{enumerate}
...
\item (0.2) List the sum of the squares of the entries of the residual vector
%
\begin{equation}
\br_\lambda = \bdG\,\bdelta\bem_\lambda-\bdelta\dvec.
\end{equation}
%
The sum of the squares of this vector is the square of the {\em misfit norm}.
...
%The misfit norm is the sum of the squares of the residuals, where the $i$th residual is $(\bG\bem-\dvec)_i$. The model norm is the sum of the squares of the model parameters $m_k$.
...
%
\begin{itemize}
\item I have pre-computed the necessary matrix $\bB$ in \tfiletomo, such that the function value $\delta\ln c(\phi_j, \theta_j)$ is computed by the dot product $\bB(j,:) \cdot \bdelta\bem$. (Index $j$ is for the plotting grid; $\bB(j,:)$ represents the $j$th row of $\bB$.) This operation is represented by \refEq{dlnv} and can be thought of as
%
\begin{equation}
...
\item (0.4) What are two ways to stabilize the inverse problem based on {\bf the experimental design}? Hint: How can we construct a matrix $\bdG^T\bdG$ with fewer zeros?
...
\item (0.5) After the first iteration, we no longer have a homogeneous ``reference'' model for phase speed. How will we need to adapt our algorithm to accommodate iterations of the phase speed model?

Hint: How will $\bdG$, $\bdelta\dvec$, and $\bdelta\bem$ change?
...
\begin{figure}
\hspace{-0.5cm}
\includegraphics[width=17cm]{socal_03b}
...
\begin{figure}[p]
\hspace{-1.5cm}
\includegraphics[width=19cm]{talk_Gik03_5_ray_hw.jpg}
...
%The value of the phase speed for the reference model is constant, so $G_{ik} = (-1/c) \int_{{\rm ray}_i} B_k\,\rmd s$. In this example, $G_{ik} = -1/(3500\,{\rm m\,s^{-1}})\;(2.45 \times 10^4\,{\rm m})  = -6.98$~s. 
\label{fig:basis}
}}
\end{figure}
