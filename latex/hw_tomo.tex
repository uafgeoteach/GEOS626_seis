...
\end{verbatim}
%
Here index $i$ is the row index (or measurement index) for $\bG$, \verb+isrc+ is the source index, and \verb+irec+ is the receiver index. For example, row $i=3293$ of $\bG$ (and $\bDelta\bd$) corresponds to the pairing of source 25 with receiver 125.
...
Note: Python is 0-indexed, so the $i$th row in the $\bG$ array will be accessed using $i-1$.
...
%
\begin{itemize}
\item I have pre-computed the necessary matrix $\bB$ in \tfiletomo, such that the function value $\delta\ln c(\phi_j, \theta_j)$ is computed by the dot product $\bB(j,:) \cdot \bem$. This operation is represented by \refEq{dlnv} and can be thought of as
%
\begin{equation}
