...
\hline\hline 
\verb+g()+ & & {\tt x,y,t,v,xr,yr} & $g_i(\bem)$ & $\nparm \times 1$ & $1 \times 1$ & \\ \hline
\verb+d()+  & & & & & & \\ \hline
\verb+G()+  & & & & & & \\ \hline
\verb+S()+  & & & & & & \\ \hline
\end{tabular}
...
\begin{table}[h]
\caption[]{{
Classification of variables for generalized least squares. In the first column, list the name of the corresponding variable in the code. Assume $\nparm$ model parameters and $\ndata$ observations (in our problem, $\ndata=12$ arrival times). Do not list numbers for the dimensions; instead use $\nparm$ and $\ndata$ (and 1).
\label{tab:vars}
}}
\begin{spacing}{1.4}
\hspace{-1cm}
